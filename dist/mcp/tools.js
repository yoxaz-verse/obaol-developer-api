...
async function executeTool(name, args, apiKey) {
    switch (name) {
        case 'get_prices': {
            if (!hasPermission(apiKey, 'prices:read'))
                deny('prices:read');
            return getPrices({ commodity: args?.commodity });
        }
        case 'get_traders': {
            if (!hasPermission(apiKey, 'traders:read'))
                deny('traders:read');
            return getTraders({ verified: args?.verified });
        }
        case 'create_inquiry': {
            if (!hasPermission(apiKey, 'inquiries:create'))
                deny('inquiries:create');
        ...
        case 'calculate_cif': {
            if (!hasPermission(apiKey, 'calculator:use'))
                deny('calculator:use');
            return calculateCif({
                fob: args?.fob,
