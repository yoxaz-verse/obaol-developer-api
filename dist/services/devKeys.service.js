...
async function updateDevKey({ developerId, keyId, label, permissions, permissionPreset, rate_limit }) {
    ...
    const row = await ApiKey.findOneAndUpdate({ _id: keyId, developerId }, { $set: update }, { new: true }).lean();
    if (!row) {
...
async function revokeDevKey({ developerId, keyId }) {
    ...
    const row = await ApiKey.findOneAndUpdate({ _id: keyId, developerId }, { $set: { is_active: false, revoked_at: new Date() } }, { new: true }).lean();
    if (!row) {
        const err = new Error('API key not found.');
        err.statusCode = 404;
        throw err;
