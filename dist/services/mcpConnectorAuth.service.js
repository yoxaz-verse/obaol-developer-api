...
async function revokeMcpConnector({ developerId, connectorId }) {
    ...
    const row = await McpConnectorToken.findOneAndUpdate({ _id: connectorId, developerId }, { $set: { is_active: false, revoked_at: new Date() } }, { new: true }).lean();
    if (!row) {
...
async function validateMcpConnector({ developerId, connectorId }) {
    ...
    if (!row.is_active || row.revoked_at)
        return { valid: false, reason: 'Connector is revoked/inactive.' };
